...
classes: wide
header:
  teaser: /assets/images/Introduction-to-iOS-Forensics/1.jpg
  overlay_image: /assets/images/Introduction-to-iOS-Forensics/1.jpg
  overlay_filter: 0.5
...
It's been 12 years since the first time I jailbroken an iOS device. That first time was with the first iPhone I ever owned-a device that brings back a rollercoaster of emotions and memories, which I'll be sharing with you as we go along.
...
While researching, I discovered that by jailbreaking the phone, you could unlock it from the carrier, regardless of the theft report, and even install Siri as a tweak from Cydia. And that's exactly how I had my first encounter with iOS security and mobile application security.

So, thank you to that guy who tricked a 15-year-old teenager... because of you, I'm here today, writing this blog with the purpose of explaining, on a technical level, the process of jailbreaking an iPhone. However, my intention is not to limit this to just using tools like checkra1n or palera1n. The goal of this blog is to give you an insight into the complex and extensive process these tools automate to achieve the chaining of vulnerabilities and exploits that ultimately result in what we could call iOS privilege escalation.


iOS jailbreaking involves exploiting security flaws in Apple’s mobile operating system to remove restrictions and gain full access to the system. This elevated access allows users to install apps and tweaks not available through Apple’s App Store, customize the interface, and modify system settings. While jailbreaking offers increased freedom, it also introduces security risks, voids warranties, and can lead to system instability if not properly managed.
...
- **2007: The Birth of Jailbreaking**  
   The first iPhone was released without an App Store, limiting its functionality to the apps provided by Apple. Hackers were quick to find ways to open the system — within months, the **iPhone Dev Team** and other hacker groups developed the first jailbreaks, allowing users to install third-party apps using the Installer.app, the precursor to Cydia. Around the same time, **George Hotz (Geohot)** gained fame for performing the first **carrier unlock** of the iPhone, a distinct but related achievement that opened the device to non-AT&T networks.
...
- **checkra1n**: Based on the checkm8 bootrom exploit, checkra1n offers an unpatchable jailbreak for iPhones from the iPhone 5s (A7) to the iPhone X (A11). Note that while the checkm8 exploit itself affects all A5–A11 devices, checkra1n requires ARM64 (AArch64) and therefore only supports A7 and later. This tool is compatible with iOS 12 and later, making it one of the most reliable options for older devices.
...
- **tfp0 (task_for_pid(0))**: Not an exploit in itself, but rather a **capability** — full read/write access to kernel memory via the kernel's task port. Various kernel exploits can grant this primitive, and it is the foundation upon which modern jailbreaks like **unc0ver** and **Taurine** build their post-exploitation chains.
...
- **SockPuppet**: A **kernel vulnerability** (CVE-2019-8605, discovered by Ned Williamson) — a use-after-free in the XNU socket subsystem. Although it is triggered from userland, the bug itself lives in the kernel. It was used in jailbreaks for iOS 12, providing kernel read/write and privilege escalation.
